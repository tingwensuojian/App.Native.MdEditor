...
- 修复 PlantUML / Infographic 在高频编辑下偶发不渲染的问题，图表显示更稳定
- 修复 Monaco 重复模块与 Worker 告警，避免回退主线程造成编辑卡顿
- 优化图表渲染策略（视口优先 + 受限并发），首屏图表展示更快
- 新增渲染耗时分段日志（主流程 / 图表增强），问题定位更直观
- 提升重型图表文档性能体验：减少误报告警并增强缓存复用
